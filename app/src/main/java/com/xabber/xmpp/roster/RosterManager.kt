...
class RosterManager(private val owner: String, private val realm: Realm) {
    ...
    // FIXED: Restored the original processing logic inside a Realm transaction
    private suspend fun processRosterBatch(iqs: List<XMPPIQ>) = withContext(Dispatchers.IO) {
        realm.write {
            iqs.forEach { iq ->
                ...
                // Pre-fetch groups and items once per IQ (original logic)
                ...
                Log.d(TAG, "Processed ${rosterQuery.items.size} roster items: $createdCount created, $updatedCount updated, $skippedCount skipped")
            }
        }
    }

    // Helper: Parse single <item> (kept for completeness, though not used in batch)
    ...
    // Handles the initial roster response (with queryIds check)
