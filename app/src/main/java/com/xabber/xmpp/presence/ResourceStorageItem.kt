...
class ResourceStorageItem : RealmObject {
    ...
    private var statusExtRaw: String = RosterItemEntity.CONTACT.rawValue
    ...
    var type: ClientType
        get() = ClientType.fromRaw(typeRaw)
        set(value) {
            typeRaw = value.rawValue
        }

    var status: ResourceStatus
        get() = ResourceStatus.fromRaw(statusRaw)
        set(value) {
            statusRaw = value.rawValue
        }
    ...
    var entity: RosterItemEntity
        get() = if (jid.contains("redmine_issue")) {
            ...
            RosterItemEntity.fromRaw(statusExtRaw)
        }
        set(value) {
            statusExtRaw = if (jid.contains("redmine_issue")) {
