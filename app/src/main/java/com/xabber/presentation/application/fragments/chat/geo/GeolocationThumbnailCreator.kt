...
class GeolocationThumbnailCreator (private val context: Context) {
    ...
    fun requestThumbnail(
        lon: Double, lat: Double, onBitmapCreated: (Bitmap) -> Unit
    ) {
    ...
    //    LogManager.i(this, "RequestThumbnail(lon: $lon, lat: $lat; pointerColor: $pointerColor)")
      ...
      XabberApplication.newInstance().run {
            ...
            val mapSnapshotable =
                MapSnapshot.MapSnapshotable { onBitmapCreated(Bitmap.createBitmap(it.bitmap)) }
